...
int main(void)
{
    ...
    // *********** Seccion : Calcular la multiplicacion de matrices en el hijo *********
    // Crear la matriz de resultado
    int Resultado[N][N];
    // Calcular la multiplicacion de las matrices
    ...
    // Escribir el resultado de la multiplicacion en la memoria compartida (tercera seccion de la memoria)
    ...
    // Creación del semáforo para esperar a que recoleccion lea el resultado de la multiplicacion
    if ((hSemaforo_resultados = CreateSemaphore(NULL, 0, 1, "Sem_Resultados_Mult")) == NULL) {
        printf("Falla al invocar CreateSemaphore: %d\n", GetLastError());
        return -1;
    }

    // Esperar a que recoleccion lea el resultado de la multiplicacion
