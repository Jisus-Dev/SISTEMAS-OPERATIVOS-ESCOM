...
int main(void)
{
    ...
    // *********** Seccion : Lectura de la matriz uno del proceso hijo *********
    ...
    // Desmapear la vista del archivo de la memoria del proceso para matrizuno
    UnmapViewOfFile(apDatosMatrizUno);
    // Cerrar el manejador de la memoria compartida para matrizuno
    CloseHandle(hMemComMatrizUno);

    // *********** Seccion : Lectura de la matriz dos del proceso hijo *********
    ...
    // *********** Seccion : Realizar la suma de matrices en el nieto *********

    int Resultado[N][N];
    SumarMatrices(Matrizuno, Matrizdos,Resultado);

    // *********** Seccion : Creacion de la memoria compartida para la matriz de resultado de la suma *********
    ...
    // Esperar hasta que el abuelo haya leído el resultado de la suma
    while (apDatosResum[0] != -1) {
        Sleep(1);
    }

    // Desmapear la vista del archivo de la memoria del proceso para el resultado de la matriz de suma
    UnmapViewOfFile(apDatosResum);
    // Cerrar el manejador de la memoria compartida para el resultado de la matriz de suma
