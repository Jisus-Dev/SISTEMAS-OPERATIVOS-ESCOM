...
void RestarMatrices(int matrizuno[N][N], int matrizdos[N][N], int resultado[N][N])
{
    ...
    {
        for (int j = 0; j < N; j++) {
            resultado[i][j] = matrizuno[i][j] - matrizdos[i][j]; // Resta los elementos correspondientes de las dos matrices.
        }
    }
}
...
int main(void)
{
    ...
    // *********** Seccion : Realizar la resta de matrices en el hijo *********

    int Resultado[N][N];
    RestarMatrices(Matrizuno, Matrizdos,Resultado);

    // *********** Seccion : Creacion de la memoria compartida para la matriz de resultado de la resta *********
    ...
    // Esperar hasta que recoleccion haya leído el resultado de la resta
    while (apDatosResrest[0] != -1) {
        Sleep(1);
    }

    // Desmapear la vista del archivo de la memoria del proceso para el resultado de la matriz de resta
    UnmapViewOfFile(apDatosResrest);
    // Cerrar el manejador de la memoria compartida para el resultado de la matriz de resta
