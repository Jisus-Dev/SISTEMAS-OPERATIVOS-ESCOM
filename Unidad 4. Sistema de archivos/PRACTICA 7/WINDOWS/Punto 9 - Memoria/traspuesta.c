...
int main(void)
{
    ...
    // *********** Seccion : Realizar la traspuesta uno *********

    int Traspuesta_uno[N][N];
    TransponerMatriz(Matrizuno,Traspuesta_uno);

    // *********** Seccion : Creacion de la memoria compartida para la matriz de resultado de la traspuesta uno *********
    ...
    // Esperar hasta que recoleccion haya leído el resultado de la traspuesta uno
    while (apDatosRestrasu[0] != -1) {
        Sleep(1);
    }

    // Desmapear la vista del archivo de la memoria del proceso para el resultado de la matriz de traspuesta uno
    UnmapViewOfFile(apDatosRestrasu);
    // Cerrar el manejador de la memoria compartida para el resultado de la matriz de traspuesta uno
    CloseHandle(hMemComRestrasu);

    // *********** Seccion : Realizar la traspuesta dos *********

    int Traspuesta_dos[N][N];
    TransponerMatriz(Matrizdos,Traspuesta_dos);

    // *********** Seccion : Creacion de la memoria compartida para la matriz de resultado de la traspuesta dos *********
    ...
    // Esperar hasta que recoleccion haya leído el resultado de la traspuesta dos
    while (apDatosRestrasd[0] != -1) {
        Sleep(1);
    }

    // Desmapear la vista del archivo de la memoria del proceso para el resultado de la matriz de traspuesta dos
    UnmapViewOfFile(apDatosRestrasd);
    // Cerrar el manejador de la memoria compartida para el resultado de la matriz de traspuesta dos
